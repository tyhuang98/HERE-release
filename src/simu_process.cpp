...
void simu_process::norm_pointcloud(cloudPoints &points) {
    ...
    Eigen::Vector3d center = points.rowwise().mean();
    // subtract mean
    Eigen::Matrix3Xd points_m = points.colwise() - center;

    double scale_max = points_m.colwise().norm().maxCoeff();

    points = points_m/scale_max;

}

void simu_process::addNoiseAndOutliers(cloudPoints &tgt, int N_OUTLIERS, std::vector<bool> &inlier_mask, double noise_bound) {
    // Add uniform noise
    ...
    for (int i = 0; i < N_OUTLIERS; ++i) {
        ...
        Eigen::Matrix<double, 3, 1> rand_vector = Eigen::Matrix<double, 3, 1>::Random();
...
void simu_process::model_data_generation(pcl::PointCloud<pcl::PointXYZ>::Ptr src_cloud_ori, int N, double tho, Eigen::Matrix4d T, double noise_bound,
    ...
    Eigen::Matrix<double, 4, Eigen::Dynamic> src_homo;
    src_homo.resize(4, src_points.cols());
    src_homo.topRows(3) = src_points;
    src_homo.bottomRows(1) = Eigen::Matrix<double, 1, Eigen::Dynamic>::Ones(N);
    Eigen::Matrix<double, 4, Eigen::Dynamic> tgt_homo = T * src_homo;
...
void simu_process::simu_data_generation(int N, double tho, Eigen::Matrix4d T, double noise_bound,
                                        cloudPoints &src_points, cloudPoints &tgt_points, std::vector<bool> &inlier_mask){

    src_points = Eigen::Matrix<double, 3, Eigen::Dynamic>::Random(3, N).array().abs();
